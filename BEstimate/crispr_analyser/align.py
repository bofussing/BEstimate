...
def print_off_targets(
    ...
    """
    summary_output = ", ".join([f"{i}: {summary[i]}" for i in range(MAX_MISSMATCHES)])
...
"""
    ...
    with open(inputfile, "rb") as in_file:
        ...
        if use_cuda & cuda.is_available():
            ...
            blocks_per_grid = guides.size + (threads_per_block - 1) // threads_per_block

            for i in range(len(args)):
                query_sequence = guides[int(args[i]) - 1]
                reverse_query_sequence = reverse_complement_binary(query_sequence, 20)
            ...
            for i in range(len(args)):
                ...
                print_off_targets(args[i], summary, off_target_ids, metadata.species_id)
